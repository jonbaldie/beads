...
func canonicalizeGitURL(rawURL string) (string, error) {
	...
	if canonical, ok := canonicalizeSCP(rawURL); ok {
		return canonical, nil
	}
	return canonicalizeLocalPath(rawURL), nil
}
...
func canonicalizeURL(rawURL string) (string, error) {
	...
	path := filepath.ToSlash(strings.TrimSuffix(strings.TrimRight(u.Path, "/"), ".git"))
	return host + path, nil
}

func canonicalizeSCP(rawURL string) (string, bool) {
	colonIdx := strings.Index(rawURL, ":")
	slashIdx := strings.Index(rawURL, "/")
	if colonIdx <= 0 || (slashIdx != -1 && colonIdx >= slashIdx) || isWindowsPath(rawURL, colonIdx) {
		return "", false
	}
	...
	path := filepath.ToSlash(strings.TrimSuffix(strings.TrimRight(parts[1], "/"), ".git"))
	return strings.ToLower(hostPart) + "/" + path, true
}

func isWindowsPath(rawURL string, colonIdx int) bool {
	return colonIdx == 1 && len(rawURL) > 2 && (rawURL[2] == '/' || rawURL[2] == '\\')
}

func canonicalizeLocalPath(rawURL string) string {
