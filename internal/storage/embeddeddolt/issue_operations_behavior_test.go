...
func TestEmbeddedIssueOperationsCreateAggregatesRelations(t *testing.T) {
	...
	result, err := operations.Create(ctx, publicops.CreateRequest{
		Actor:                   "writer",
		ForceIDPrefix:           true,
		Issue:                   &types.Issue{IssueID: types.IssueID{ID: "ops-aggregate"}, IssueContent: types.IssueContent{Title: "aggregate"}, IssueWorkflow: types.IssueWorkflow{Status: types.StatusOpen, Priority: 2, IssueType: types.TypeTask}},
		ParentID:                parent.ID,
...
func TestEmbeddedIssueOperationsUpdateClaimCASAndTransferMatrix(t *testing.T) {
	...
	create := func(id, assignee string, status types.Status, wisp bool) *types.Issue {
		issue := &types.Issue{IssueID: types.IssueID{ID: id}, IssueContent: types.IssueContent{Title: id}, IssueWorkflow: types.IssueWorkflow{Assignee: assignee, Status: status, Priority: 2, IssueType: types.TypeTask}, IssueWisp: types.IssueWisp{Ephemeral: wisp}}
		if err := te.store.CreateIssue(ctx, issue, "seed"); err != nil {
			t.Fatal(err)
		}
		return issue
...
func TestEmbeddedIssueOperationsCloseAndReopenAWisp(t *testing.T) {
	...
	wisp := &types.Issue{IssueID: types.IssueID{ID: "ops-wisp"}, IssueContent: types.IssueContent{Title: "wisp"}, IssueWorkflow: types.IssueWorkflow{Status: types.StatusOpen, Priority: 2, IssueType: types.TypeTask}, IssueWisp: types.IssueWisp{Ephemeral: true}}
...
func TestEmbeddedIssueOperationsPersistenceRollbackWispAndDetachedResults(t *testing.T) {
	...
	for _, transition := range []struct{ from, to publicops.PersistenceMode }{{publicops.PersistenceModePersistent, publicops.PersistenceModeEphemeral}, {publicops.PersistenceModeEphemeral, publicops.PersistenceModeNoHistory}, {publicops.PersistenceModeNoHistory, publicops.PersistenceModePersistent}} {
		...
		if err := te.store.CreateIssue(ctx, &types.Issue{IssueID: types.IssueID{ID: id}, IssueContent: types.IssueContent{Title: id}, IssueWorkflow: types.IssueWorkflow{Status: types.StatusOpen, Priority: 2, IssueType: types.TypeTask}, IssueWisp: types.IssueWisp{Ephemeral: transition.from == publicops.PersistenceModeEphemeral, NoHistory: transition.from == publicops.PersistenceModeNoHistory}}, "seed"); err != nil {
			t.Fatal(err)
		}
