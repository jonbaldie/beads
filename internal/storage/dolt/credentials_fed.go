...
func (s *DoltStore) prepareCLIRouteForCloudAuth(ctx context.Context, remote string) (bool, error) {
	...
	remoteURL, err := s.remoteURLForCLIRoute(ctx, remote)
	if err != nil {
		return false, err
	}
	if remoteURL == "" {
	...
	if len(prefixes) == 0 || !hasCloudCredential(prefixes) {
		return false, nil
	}
...
func (s *DoltStore) remoteURLForCLIRoute(ctx context.Context, remote string) (string, error) {
	remotes, err := s.ListRemotes(ctx)
	if err != nil {
		return "", fmt.Errorf("list Dolt remotes before cloud-auth routing for remote %q: %w", remote, err)
	}
	for _, r := range remotes {
		if r.Name == remote {
			return r.URL, nil
...
func hasCloudCredential(prefixes []string) bool {
