...
func (s *DoltStore) DependencyEditor() (issueops.DependencyEditor, error) {
	return NewDependencyEditor(s)
}

// NewDependencyEditor returns a guarded dependency editor backed by store.
func NewDependencyEditor(store *DoltStore) (issueops.DependencyEditor, error) {
	if store == nil {
...
type dependencyEditor struct{ store *DoltStore }
